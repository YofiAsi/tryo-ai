...
class BatchTask(Document):
    ...
    def endpoint(self) -> str:
        """Get the API endpoint for this batch task type."""
        return TypeToEndpoint[self.task_type]

    # Status and progress
    status: BatchTaskStatus = Field(default=BatchTaskStatus.CREATED)

    # Batch management
    batch_ids: List[PydanticObjectId] = Field(default_factory=list, description="List of associated batch IDs")
    ...
    def add_batch(self, batch: Batch) -> None:
        ...
        if not batch.id:
            raise ValueError("Batch has no ID")
